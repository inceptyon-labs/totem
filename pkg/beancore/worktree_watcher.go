...
// worktreeWatcher tracks a single worktree's .totem/ directory.
type worktreeWatcher struct {
	worktreePath string // root of the worktree (e.g., ~/.totem/worktrees/<project>/<id>)
	beansDir     string // .totem/ dir inside the worktree
	done         chan struct{}
}

// WatchWorktreeBeans starts watching a worktree's .totem/ directory for bean changes.
// When beans change in the worktree, they are merged into the runtime state as dirty
// (not persisted to the main repo's disk).
// Returns nil if the worktree's .totem/ directory doesn't exist.
func (c *Core) WatchWorktreeBeans(worktreePath string) error {
	beansDir := filepath.Join(worktreePath, BeansDir)

	// Check if the worktree has a .totem/ directory
	if _, err := os.Stat(beansDir); os.IsNotExist(err) {
		return nil // No .totem/ dir in this worktree, nothing to watch
	}
...
// loadWorktreeBeansInitial scans a worktree's .totem/ directory and loads
...
// UnwatchWorktreeBeans stops watching a worktree's .totem/ directory.
...
// UnwatchAllWorktrees stops watching all worktree .totem/ directories.
...
func (c *Core) worktreeWatchLoop(wt *worktreeWatcher, watcher *fsnotify.Watcher) {
	...
	for {
		...
		case event, ok := <-watcher.Events:
			...
			// Verify the file is within the worktree's .totem/ directory
...
func (c *Core) handleWorktreeChanges(wt *worktreeWatcher, changes map[string]fsnotify.Op) {
	...
	for path, op := range changes {
		...
		if isDelete && !isCreateOrWrite {
			...
			// Try to load from the main .totem/ directory.
...
// findMainBeanFile searches the main .totem/ directory for a file matching the
