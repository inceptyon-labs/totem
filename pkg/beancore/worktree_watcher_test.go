...
func TestWorktreeWatcher(t *testing.T) {
	t.Run("watches worktree totems dir and merges changes", func(t *testing.T) {
		...
		// Create a fake worktree directory with a .totem/ subdir
		wtDir := t.TempDir()
		wtBeansDir := filepath.Join(wtDir, BeansDir)
		if err := os.MkdirAll(wtBeansDir, 0755); err != nil {
			t.Fatalf("failed to create worktree .totem dir: %v", err)
		}
...
`
	...
	t.Run("does not crash when worktree has no .totem dir", func(t *testing.T) {
		core, _ := setupTestCore(t)
		...
		if err != nil {
			t.Errorf("WatchWorktreeBeans() should return nil for missing .totem/ dir, got %v", err)
		}
	})
