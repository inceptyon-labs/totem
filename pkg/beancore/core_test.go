...
func TestInit(t *testing.T) {
	...
	if err != nil {
		t.Fatalf(".totem directory not created: %v", err)
	}
	if !info.IsDir() {
		t.Error(".totem is not a directory")
	}
}
...
func TestFullPath(t *testing.T) {
	...
	got := core.FullPath(b)
	want := "/path/to/.totem/abc1--test.md"
...
func TestGetFromArchive(t *testing.T) {
	...
	t.Run("no archive directory", func(t *testing.T) {
		...
		if err := os.MkdirAll(freshBeansDir, 0755); err != nil {
			t.Fatalf("failed to create .totem dir: %v", err)
		}
