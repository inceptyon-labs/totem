...
// Watch starts watching the .totem directory for changes.
...
// Unwatch stops watching the .totem directory.
...
func (c *Core) watchLoop(watcher *fsnotify.Watcher) {
	...
	for {
		...
		case event, ok := <-watcher.Events:
			...
			// Only care about .md files within the .totem directory tree
			if !strings.HasSuffix(event.Name, ".md") {
				continue
			}

			// Verify the file is within the .totem directory
