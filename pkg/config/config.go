...
type WorktreeConfig struct {
	...
	// Default: ~/.totem/worktrees/<project-name>/
...
// FindConfig searches upward from the given directory for a .totem.yml config file.
// Returns the absolute path to the config file, or empty string if not found.
func FindConfig(startDir string) (string, error) {
	return FindConfigWithin(startDir, "")
}

// FindConfigWithin searches upward from startDir for a .totem.yml config file,
...
func (c *Config) toYAMLNode() *yaml.Node {
	...
	if c.Beans.Path != "" {
		key := strNode("path")
		key.HeadComment = "Directory where totem files are stored"
	...
	if c.Beans.DefaultStatus != "" {
		...
		key.HeadComment = "Default status for new totems"
	...
	if c.Beans.DefaultType != "" {
		...
		key.HeadComment = "Default type for new totems"
	...
	if c.Worktree.Path != "" {
		key := strNode("path")
		key.HeadComment = "Directory for worktrees (default: ~/.totem/worktrees/<project>/)"
	...
	if c.Server.Port != 0 {
		...
		portKey.HeadComment = "Port for the web UI (used by `totem-serve`)"
	...
	topMapping := &yaml.Node{
		...
		Tag:         "!!map",
		HeadComment: "Totem configuration\nSee: https://github.com/inceptyon-labs/totem",
	}
...
// Otherwise, defaults to ~/.totem/worktrees/<projectName>/.
...
func (c *Config) ResolveWorktreePath(projectName string) (string, error) {
	...
	return filepath.Join(home, ".totem", "worktrees", projectName), nil
}
