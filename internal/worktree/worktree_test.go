...
// a .totem directory inside it, and a separate worktree root directory.
func initTestRepo(t *testing.T) (repoDir, beansDir, worktreeRoot string) {
	...
	bd := filepath.Join(dir, ".totem")
	if err := os.MkdirAll(bd, 0755); err != nil {
		t.Fatalf("MkdirAll .totem: %v", err)
	}
...
branch refs/heads/totems/totems-foo`,
		...
		},
		{
			...
			worktreesDir: "/home/user/.totem/worktrees/project",
			input: `worktree /home/user/project
HEAD abc123
branch refs/heads/main
...
func TestDetectBeanIDs(t *testing.T) {
	...
	// Commit a file in .totem so the directory exists on main
	if err := os.WriteFile(filepath.Join(beansDir, ".gitkeep"), []byte(""), 0644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	gitRun(t, repoDir, "add", ".totem/.gitkeep")
	gitRun(t, repoDir, "commit", "-m", "add .totem dir")
	...
	// Create bean files in the worktree's .totem/ directory
	wtBeansDir := filepath.Join(wt.Path, ".totem")
	...
	// Add a file in .totem/.worktrees/ (should be excluded)
	...
	// Add a file in .totem/archive/ (should be excluded)
