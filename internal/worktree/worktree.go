...
type Manager struct {
	repoRoot     string
	worktreeRoot string // directory where worktrees are created (e.g. ~/.totem/worktrees/<project>/)
	baseRef      string
...
// worktreeRoot is the directory where worktrees are created (e.g. ~/.totem/worktrees/<project>/).
...
// worktreesDir is the path to the beans worktrees directory (e.g. "~/.totem/worktrees/<project>/"),
...
// It filters for .totem/*.md files, excluding dot-prefixed subdirs (like .worktrees/,
...
func (m *Manager) DetectBeanIDs(worktreePath string) []string {
	...
	for _, change := range changes {
		// Must be directly under .totem/ and end with .md
		rest, ok := strings.CutPrefix(change.Path, ".totem/")
		if !ok {
			continue
		}
		...
		fullPath := filepath.Join(worktreePath, ".totem", rest)
...
// (default: ~/.totem/worktrees/<project>/).
